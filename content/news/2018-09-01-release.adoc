...
emits defensive JavaScript that checks the return value of `big?`, coercing 
it to a Boolean value by using `cljs.core.truth_`:
...
value, and therefore so does `big?`, and dispenses with the check:
...
With this kind of inference, it is no longer necessary to add a `^boolean`
...
you will now see a warning that reflects the type:
...
Previously, without an explcit type hint, the compiler would produce a 
warning that indicate a type of `any` instead of `number`.
