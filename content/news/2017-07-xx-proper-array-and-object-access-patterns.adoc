= Proper Array and Object Access Patterns
Mike Fikes
2017-07-xx
:jbake-type: post
...
safety mechanisms to ClojureScript's `aget` (perhaps only at dev time, elided at runtime). But 
...
Towards that end, the upcoming ClojureScript release includes a new optional 
...
In the future setting this warning may also cause emissions of 
checked array access during development as we suggested above.

This warning can be enabled
in order to highlight instances where these
...
To enable the warning, simply add

[source,clojure]
```
:warnings {:invalid-array-access true}
```
