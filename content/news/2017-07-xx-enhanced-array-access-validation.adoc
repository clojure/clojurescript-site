= Enhanced Array Access Validation
Mike Fikes
2017-07-xx
:jbake-type: post
...
Additionally, the values passed to `aget` and `aset` at runtime
are checked. If `:checked-arrays` is set to `:warn`, warning
logs will be generated when bad values are passed, and if set to `:error`,
