...
The ClojureScript needs the `JSONStream` and `module-deps` packages to
build the project.
...
No let's make a CommonJS file that will load deps from `node_modules`:
...
Notice that a new helper, `cljs.build.api/node-inputs`. Given a CommonJS
foreign lib entry, the ClojureScript compiler will now invoke a Node.js script
to figure out all the `node_module` dependencies needed by this entry point and
return a vector of CommonJS foreign lib entries.
...
We need this two additional externs because internally React refers to the
`process` module and React internally relies on the string names of the plugins.
`:pseudo-names true` is a good way to figure out cases like this.
