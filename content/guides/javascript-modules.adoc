...
from Cljsjs provides the necessary JavaScript file and a implementation for
`cljsjs.closure/js-transforms` multimethod, which is used for foreign libraries using `:preprocess`
value `:cljsjs.babel-standalone/babel`.
The multimethod implementation uses Nashorn JS engine to run Babel and process foreign libraries.
...
In previous example the Babel transformation was provided by a Cljsjs package,
which also provided the implementation for `cljs.closure/js-transforms` multimethod.
If you need to use different transformations you can write your own implementation
for this multimethod. The Babel transformation can be implemented like this, without
using the Cljsjs package:
...
In this example the `:preprocess` key for modules is `:jsx` which refers to the
definded multimethod.
