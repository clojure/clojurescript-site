...
** `-ro`, `--repl-opts` _edn_ Options to configure the repl-env
...
** `-c`, `--compile` _ns_ Compile a namespace. If `--repl` present after namespace will launch a REPL after the compile completes
...
init options:
  -co, --compile-opts edn     Options to configure the build
  ...
  -re, --repl-env env         The REPL environment to use. Built-in supported
                              ...
                              to browser
  -ro, --repl-opts edn        Options to configure the repl-env
...
main options:
   -                          Run a script from standard input
   -c, --compile ns           Compile a namespace. If --repl present after
