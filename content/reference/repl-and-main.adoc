...
* init options:
** `-cf`, `--compile-opts` _edn_ Options to configure the build
...
** `-rf`, `--repl-opts` _edn_ Options to configure the repl-env
** `-t`, `--target` _name_ The JavaScript target. Configures environment bootstrap and defaults to `browser`. Supported values: `nodejs`, `nashorn`, `webworker`, `none`
...
** `-O`, `--optimizations` _level_ Set optimization level, only effective with `-c` main option. Valid values are: `none`, `whitespace`, `simple`, `advanced`
** `-o`, `--output-to` _file_ Set the output compiled file
** `-w`, `--watch` _path_ Continuously build, only effective with `-c` main option
* main options:
** `-` Run a script from standard input
** `-c`, `--compile` _ns_ Compile a namespace. If `-r` / `--repl` present after namespace will launch a REPL after the compile completes
...
init option:
  -cf, --compile-opts edn     Options to configure the build
  ...
  -re, --repl-env             The REPL environment to use. Built-in supported
                              ...
                              to nashorn
  -rf, --repl-opts edn        Options to configure the repl-env
